...
void MX_UART7_Init(void)
{
  ...
  /* USER CODE BEGIN UART7_Init 2 */
    __HAL_UART_ENABLE_IT(&huart7,UART_IT_IDLE);//???????6 ???§ß????§Ø?
...
//    __HAL_UART_ENABLE_IT(&huart8,UART_IT_IDLE);//???????6 ???§ß????§Ø?
//    HAL_UART_Receive_DMA(&huart8,(uint8_t *)&BlueTooth_data,RECEIVE_DATA_SIZE);//??????6 DMA????
...
void MX_USART6_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART6_Init 2 */
    __HAL_UART_ENABLE_IT(&huart6,UART_IT_IDLE);//???????6 ???§ß????§Ø?
    HAL_UART_Receive_DMA(&huart6,(uint8_t *)&receive_data,RECEIVE_DATA_SIZE);//??????6 DMA????
